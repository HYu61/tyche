...
let List = function () {
    ...
    let handleList = function () {
        ...
        jqGrid.jqGrid({
            caption: "Tyche Vlog User List",
            url: hdnContextPath + "/users/list.action",
            mtype: "get",
            styleUI: 'Bootstrap',//设置jqgrid的全局样式为bootstrap样式
            datatype: "json",
            colNames: ['ID', 'Profile Photo', 'Username', 'Nickname', 'Followers', 'Follows', 'Like Count'],
            colModel: [
                {name: 'id', index: 'id', width: 30, sortable: false, hidden: false},
                {
                    name: 'profilePhoto', index: 'username', width: 30, sortable: false,
                ...
                {name: 'followerCounts', index: 'age', width: 20, sortable: false},
                {name: 'followCounts', index: 'sexValue', width: 20, sortable: false},
                {name: 'receivedLikeCounts', index: 'province', width: 20, sortable: false, hidden: false}
            ],
